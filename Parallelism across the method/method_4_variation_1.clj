...
(defn calc-new-value [func iteration storage-func]
  (apply (peek func) (map #(deref ((deref (deref %)) iteration)) (butlast storage-func))))
...
(defn create-value-map [func iterations]
  (let [m (zipmap (range) (repeatedly* () (+ iterations 1) promise))]
    (deliver (m 0) (first func)) 
    m))
...
(defn define-vars [names funcs iterations]
    (doall (map #(intern *ns* (symbol %) (atom (create-value-map %2 iterations))) names funcs)))
...
(defn synchronous-shared [start end step & funcs]
  (let [iterations (long (quot (- end start) step))
    ...
    (doall 
      (map #(deref ((deref (deref %)) iterations)) vars))))
